...
const insertVideoRecord = async (video: any) => {
...
const updateVideoRecordStatus = async (video: any) => {
  const user = await getUser();
  if (!user) {
    throw new Error('User not logged in.');
  }
...
const getVideoRecord = async (videoId: string) => {
  const { data: video, error } = await supabaseAdmin.from('videos').select('*').eq('video_id', videoId).single();
  if (error) {
    throw new Error(`Video lookup failed: ${error.message}`);
  }
  return video;
}
...
const getVideoRecords = async () => {
  ...
  const { data: videos, error } = await supabaseAdmin.from('videos').select('*').eq('user_id', user.id);
  if (error) {
    throw new Error(`Video lookup failed: ${error.message}`);
  }
  return videos;
